...
from typing import Iterable, Any, Iterator, Callable, Coroutine, Tuple, Dict, Union
...
#         return keyboard
#
#     def _check_dp(self, data) -> tuple[tuple[Any, Any], dict[str, State | str]]:
#         if not self.dp:
#             return (data, F.text.startswith(self._startswith)), \
#                 {'state': self._state if self._state else '*'}
#         else:
#             self.dp.callback_query.register(
#                 data,
#                 F.text.startswith(self._startswith),
#                 **{'state': self._state if self._state else '*'}
#             )
#
#     def paginator_handler(self) -> tuple[
#         tuple[Callable[[CallbackQuery], Coroutine[Any, Any, None]], F.text],
#         dict[str, State | str]
#     ]:
#         """
#         Example:
